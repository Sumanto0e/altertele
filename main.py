...
def ping(message):
    ...
    if message.text == "/setrole":
        ...
        if bal >= ROLING:
            ...
            bot.send_message(
                user_id, f"_❌Koin anda kurang, setidaknya memiliki {ROLING} {TOKEN} untuk transfer_", parse_mode="Markdown")
        ...
        if len(pulu) >= 3:
            if ah == True:
                if data['checkin'][user] >= 1:
                    ...
                    bot.send_message(PANTAU, "username : @{}\n\nid : {}\n\n teks : {}".format(
                        message.from_user.username, user, lalo))
